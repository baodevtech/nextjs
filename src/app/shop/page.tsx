...
const BenefitIcon = ({ src, FallbackIcon, colorClass, bgClass }: any) => {
  if (src) {
    return (
      <div
        className={`w-8 h-8 rounded-full ${bgClass} flex items-center justify-center overflow-hidden shrink-0`}
      >
  ...
  return (
    <div
      className={`w-8 h-8 rounded-full ${bgClass} flex items-center justify-center ${colorClass} shrink-0`}
    >
      ...
      <div className="mb-10 bg-gradient-to-br from-brand-50 via-white to-brand-50 rounded-2xl p-8 md:p-12 border border-brand-100 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="relative z-10 hidden md:grid grid-cols-2 gap-3 min-w-[280px]">
          ...
          <div className="bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-sm border border-brand-100 flex flex-col items-center text-center gap-2 hover:transform hover:scale-105 transition-transform duration-300">
            ...
            <div>
              <p className="font-bold text-xs text-slate-900">
                {b1?.heading || "Bảo hành 15 năm"}
              </p>
              <p className="text-[10px] text-slate-500">
                {b1?.subHeading || "Chính hãng"}
              </p>
            </div>
          </div>
          {/* Benefit 2 */}
          <div className="bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-sm border border-brand-100 flex flex-col items-center text-center gap-2 hover:transform hover:scale-105 transition-transform duration-300">
            ...
            <div>
              <p className="font-bold text-xs text-slate-900">
                {b2?.heading || "Giao hàng 24h"}
              </p>
              <p className="text-[10px] text-slate-500">
                {b2?.subHeading || "Toàn quốc"}
              </p>
            </div>
          </div>
          {/* Benefit 3 */}
          <div className="col-span-2 bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-sm border border-brand-100 flex items-center gap-4 hover:transform hover:scale-105 transition-transform duration-300">
            ...
            <div className="text-left">
              <p className="font-bold text-xs text-slate-900">
                {b3?.heading || "Đa dạng mẫu mã"}
              </p>
              <p className="text-[10px] text-slate-500">
                {b3?.subHeading || "500+ Texture"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative z-10 p-8 md:p-12 w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div className="max-w-2xl">
          ...
          <div className="flex items-center gap-4">
            <Button
              variant="primary"
              className="py-2 px-4 text-xs bg-brand-500 text-white hover:bg-slate-100 border-none"
            >
        ...
        <div className="hidden md:flex flex-col gap-3 min-w-[200px]">
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl text-white">
            <p className="text-xs text-brand-200 font-bold uppercase tracking-wider mb-1">
  ...
  return (
    <div className="border-b border-gray-100 py-5 last:border-0">
      ...
      >
        ...
        <ChevronDown
          size={16}
          className={`text-slate-400 group-hover:text-brand-600 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </button>
...
function ShopContent() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // ✅ ĐẾM INSTOCK ĐÚNG
      const instock = prods.filter((p) => p.stockStatus === "IN_STOCK").length;
  ...
  const filteredProducts = products.filter((p) => {
    ...
    return (
      matchCategory && matchBrand && matchStock && matchPromo && matchSearch
    );
  });
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <ShopHeader
          ...
          productCount={
            currentCategory ? filteredProducts.length : inStockCount
          }
            ...
            <div className="lg:sticky lg:top-24 bg-white lg:rounded-2xl lg:border border-gray-100 lg:shadow-xl lg:shadow-gray-200/40 p-5 space-y-2 h-full lg:h-auto lg:max-h-[85vh] overflow-y-auto no-scrollbar">
              ...
              <FilterSection title="Danh Mục">
                <div className="space-y-1">
                  ...
                  >
                    <span
                      className={`text-sm ${filter === "all" ? "font-bold text-brand-700" : "font-medium text-slate-600"}`}
                    >
                      Tất cả
                    </span>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${filter === "all" ? "bg-white text-brand-700 font-bold shadow-sm" : "bg-gray-100 text-gray-500"}`}
                    >
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`text-sm ${isActive ? "font-bold text-brand-700" : "font-medium text-slate-600"}`}
                        >
                          {cat.name}
                        </span>
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${isActive ? "bg-white text-brand-700 font-bold shadow-sm" : "bg-gray-100 text-gray-500"}`}
                        >
                          {count}
                        </span>
                      </div>
              ...
              <FilterSection title="Thương Hiệu">
                <div className="space-y-1">
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${brandFilter === "all" ? "border-brand-600 bg-brand-600" : "border-gray-300"}`}
                    >
                      {brandFilter === "all" && (
                    ...
                    <span
                      className={`text-sm flex-1 ${brandFilter === "all" ? "font-bold text-brand-700" : "text-slate-600"}`}
                    >
                      Tất cả
                    </span>
                  </div>
                  {brands.map((brand) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${isActive ? "border-brand-600 bg-brand-600" : "border-gray-300 group-hover:border-brand-400"}`}
                        >
                          {isActive && (
                            <div className="w-1.5 h-1.5 bg-white rounded-full"></div>
                          )}
                        ...
                        <span
                          className={`text-sm flex-1 ${isActive ? "font-bold text-brand-700" : "text-slate-600"}`}
                        >
                          {brand}
                        </span>
              ...
              <FilterSection title="Trạng Thái">
                <div className="space-y-2">
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${inStockOnly ? "bg-brand-600 border-brand-600" : "border-gray-300 bg-white"}`}
                    >
                      {inStockOnly && (
                    ...
                    <span
                      className={`text-sm ${inStockOnly ? "font-bold text-brand-700" : "text-slate-600"}`}
                    >
                      Chỉ hiện còn hàng
                    </span>
                  </div>
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isPromotion ? "bg-brand-600 border-brand-600" : "border-gray-300 bg-white"}`}
                    >
                      {isPromotion && (
                        <Check size={12} className="text-white" />
                      )}
                    ...
                    <span
                      className={`text-sm ${isPromotion ? "font-bold text-brand-700" : "text-slate-600"}`}
                    >
                      Đang giảm giá
                    </span>
                  </div>
          ...
          <div className="flex-1">
            <div className="flex flex-wrap gap-4 justify-between items-center mb-6">
              ...
              <div className="hidden md:flex items-center gap-2">
                {[
                  ...
                  <button
                    key={i}
                    className={`px-4 py-2 rounded-full text-xs font-medium transition-all ${i === 0 ? "bg-slate-900 text-white shadow-md" : "bg-white border border-gray-200 text-slate-600 hover:border-brand-500 hover:text-brand-600"}`}
                  >
                    {sort}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button className="p-2 bg-slate-100 text-slate-900 rounded-md hover:bg-slate-200 transition-colors">
                  <LayoutGrid size={18} />
                </button>
                <button className="p-2 text-slate-400 hover:text-slate-600 hover:bg-gray-50 rounded-md transition-colors">
                  <List size={18} />
                </button>
              </div>
            </div>
            ...
            {!loading && filteredProducts.length === 0 && (
              <div className="text-center py-32 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                ...
                <button
                  onClick={resetFilters}
                  className="text-brand-600 font-bold text-sm hover:underline flex items-center justify-center gap-2 mx-auto"
                >
                ...
                <div className="lg:w-3/4">
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: currentCategory.bottomContent,
                    }}
                  />
                </div>
              </div>
